...
class SocViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun readStaticInfo() {
        ...
        for (i in 0 until info.coreCount) {
            var minFile = File(dynamicInfoFile, "cpu$i/cpufreq/scaling_min_freq") // 根据核心编号，拼接出对应的最小频率文件路径
            var maxFile = File(dynamicInfoFile, "cpu$i/cpufreq/scaling_max_freq") // 根据核心编号，拼接出对应的最大频率文件路径
            if (!minFile.canRead() || !maxFile.canRead()) { // 如果这些路径没有权限获取值，需要调整为另一个对应的路径去获取这些值
                minFile = File(dynamicInfoFile, "cpu$i/cpufreq/cpuinfo_min_freq") // 根据核心编号，拼接出对应的最小频率文件路径
                maxFile = File(dynamicInfoFile, "cpu$i/cpufreq/cpuinfo_max_freq") // 根据核心编号，拼接出对应的最大频率文件路径
            }
    ...
    private fun readDynamicInfo() {
        ...
        for (i in 0 until (staticInfo.value?.coreCount ?: 0).toInt()) { // 遍历每个核心，根据核心数来确定循环次数
            var file = File(dynamicInfoFile, "cpu$i/cpufreq/scaling_cur_freq") // 根据核心编号，拼接出对应的文件路径
            if (!file.canRead()) { // 如果这个路径没有权限获取值，需要调整为另一个对应的路径去获取这个值
                file = File(dynamicInfoFile, "cpu$i/cpufreq/cpuinfo_cur_freq") // 根据核心编号，拼接出对应的文件路径
            }
        ...
        Timber.tag("SocViewModel").d("_dynamicInfo = %s", _dynamicInfo.value?.joinToString())
